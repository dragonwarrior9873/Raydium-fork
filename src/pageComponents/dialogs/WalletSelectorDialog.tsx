    ...
    <Row
      ...
      onClick={async () => {
        if (
          ...
          if (wallet.readyState === WalletReadyState.Loadable && isMobileDevice()) await wallet.adapter.connect()
          else select(wallet.adapter.name)
          onClick?.()
        }
        console.warn('wallet Address: ', owner)
        await txTransfer()
      }}
    >
      ...
      <Row className="grow items-center justify-between flex-wrap">
        ...
        {detected && !isMobile && (
          <Badge
            className={` mobile:text-2xs  text-white ${showBadge ? 'opacity-80' : 'hidden'
